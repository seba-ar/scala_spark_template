...
object CdrData {
  ...
  def selectOrder(df: DataFrame): DataFrame = {
    df.select(
      ...
      col("start_time_of_billcycle").isNotNull && col(
        "start_time_of_billcycle"
      ) =!= "" &&
        expr(
          "CAST(SUBSTRING(start_time_of_billcycle, 1, 6) AS INT)"
        ).isNotNull &&
        col("calling_party_imsi").isNotNull && col(
          "calling_party_imsi"
        ) =!= "" &&
        col("calling_party_number").isNotNull && col(
          "calling_party_number"
        ) =!= "" &&
...
object CdrDataError {
  def getTransformations: Map[String, Column] = Map(
    ...
    "error_type" -> expr(
      """CASE 
      WHEN (start_time_of_billcycle is NULL or start_time_of_billcycle = "") THEN "start_time_of_billcycle"
      WHEN (calling_party_imsi is NULL or calling_party_imsi = "") THEN "calling_party_imsi"
      WHEN (calling_party_number is NULL or calling_party_number = "") THEN "calling_party_number"
      WHEN (main_offering_id is NULL or main_offering_id = "") THEN "main_offering_id"
      WHEN (rating_group is NULL or rating_group = "") THEN "rating_group"
      WHEN (be_id is NULL or be_id = "") THEN "be_id"
      ELSE null
    END"""
    ),
    "load_error_date" -> expr(
      "cast(date_format(now(), \"yyyyMMddHH\") AS INT)"
    )
  )
  ...
  def selectOrder(df: DataFrame): DataFrame = {
    df.select(
      ...
      col("start_time_of_billcycle").isNull || col(
        "start_time_of_billcycle"
      ) === "" ||
        expr("CAST(SUBSTRING(start_time_of_billcycle, 1, 6) AS INT)").isNull ||
        col("calling_party_imsi").isNull || col("calling_party_imsi") === "" ||
        col("calling_party_number").isNull || col(
          "calling_party_number"
        ) === "" ||
        col("main_offering_id").isNull || col("main_offering_id") === "" ||
        col("rating_group").isNull || col("rating_group") === "" ||
        col("be_id").isNull || col("be_id") === ""
    )
  }
}
